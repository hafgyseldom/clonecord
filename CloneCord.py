...
# CloneCord Bot by !.₭₳☈.!#9600 with help from REKULOUS Tasky Lizard & Razorback! GClone made by Donwa on GitHub. Original Discord "GClone-Bot" by KushTheApplusser!

# Bot tested by me, MineRocker, Saaz, REKULOUS, and KushTheApplusser. Inspired by Telegram GClone, GDrive, and Mirror Bots.
...
os.system(
    'title CloneCord Discord Bot by !.₭₳☈.!#9600. Original code by REKULOUS and KushTheApplusser')
os.system('color 0B')
...
@bot.event
async def on_ready():
    print('<===============================|| Running CloneCord ||===============================>')
    print('Connected to bot: {}'.format(bot.user.name))
    print('Bot ID: {}'.format(bot.user.id))
    print('CloneCord is Ready!')
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=PREFIX + "help to get help! CloneCord"))
...
async def help(ctx, command: Optional[str]):
    ...
    helpEmbed.set_footer(
        text="Bot originally created by Kush The A++er#2976, REKULOUS#5580, Pratyush.#6969 and Razorback#4637.  Revamped version by !.₭₳☈.!#9600. Thanks everyone for the help!",
    ...
    if command:
        for com in list_of_commands:
            if com["command"] == command:
                ...
                embed1.set_author(
                    name="CloneCord", icon_url="https://1.bp.blogspot.com/-M5PLcSana6M/XgBHF7jUjiI/AAAAAAAAUzs/S24qhuijluwKlzIOnc2gntoI-U83ZsrJACLcBGAsYHQ/s1600/rclone_logo.png")
                embed1.set_footer(text="Just paste source and destination Google Drive Links.",
...
# Link to ID conversion
def srcid(source):
    x = re.search(r"[/,=]1[a-z,A-Z,\-,_,0-9]*", source)
    sre = x.group()
    if sre[0]=='/':
        sre = sre.split("/")
    elif sre[0]=='=':
    ...
    s1 = "{" + sre[-1] + "}"
    return s1
def desid(destination):
    y = re.search(r"[/,=]1[a-z,A-Z,\-,_,0-9]*", destination)
    desti = y.group()
    if desti[0]=='/':
        desti = desti.split("/")
    elif desti[0]=='=':
...
async def clone(ctx, source, destination):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "copy", f"GC:{s}", f"GC:{d}", "--transfers", "50", "-vP", "--stats-one-line", "--stats=15s", "--ignore-existing", "--drive-server-side-across-configs", "--drive-chunk-size", "128M", "--drive-acknowledge-abuse", "--drive-keep-revision-forever"
    )
    await proc.wait()
    await ctx.send("**Cloning Complete** ---{}".format(destination))
    print(
        "===========================================================================[CLONING COMPLETE]===========================================================================")
...
async def move(ctx, source, destination):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "move", f"GC:{s}", f"GC:{d}", "--transfers", "50", "--tpslimit-burst", "50" "--checkers", "10", "-vP", "--stats-one-line", "--stats=15s", "--ignore-existing", "--drive-server-side-across-configs", "--drive-chunk-size", "128M", "--drive-acknowledge-abuse", "--drive-keep-revision-forever", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**File Transfers Completed** ---{}".format(destination))
    print(
...
async def sync(ctx, source, destination):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "sync", f"GC:{s}", f"GC:{d}", "--transfers", "50", "--tpslimit-burst", "50", "--checkers", "10", "-vP", "--stats-one-line", "--stats=15s", "--drive-server-side-across-configs", "--drive-chunk-size", "128M", "--drive-acknowledge-abuse", "--drive-keep-revision-forever", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**Sync Completed** ---{}".format(destination))
    print(
        "===========================================================================[SYNC COMPLETED]===========================================================================")
...
async def emptdir(ctx, source):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "delete", f"GC:{s}", "-vP", "--drive-trashed-only", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**Emptying Directory Completed** --- {}".format(source))
    print(
...
async def md5(ctx, source):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "md5sum", f"GC:{s}", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**Finished Producing MD5** ---{}".format(source))
    print(
...
async def rmdi(ctx, source):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "rmdirs" f"GC:{s}", "-v", "--stats-one-line", "--stats=15s", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**Finished removing empty dirs** --- {}".format(source))
    print(
...
async def dedupe(ctx, source):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "dedupe", "--dedupe-mode", "newest", f"GC:{s}", "-v", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**Finished Dedupe** --- {}".format(source))
    print(
...
async def mkdir(ctx, source):
    ...
    await proc.wait()
    await ctx.send("**Created directory** --- {}".format(source))
    print(
        "===========================================================================[CREATED DIRECTORY]===========================================================================")
...
async def purge(ctx, source):
    ...
    proc = await asyncio.create_subprocess_exec(
        "gclone", "purge", f"GC:{s}", "-vP", "--stats-one-line", "--stats=15s", "--fast-list"
    )
    await proc.wait()
    await ctx.send("**Purged Directory** --- {}".format(source))
    print(
        "===========================================================================[PURGED DIRECTORY]===========================================================================")
